...
def generate_markdown_report(
    ...
    lines = [
        ...
        f"- **Суммарная загрузка маршрутов горловины:** {metrics.get('throat_utilization', 0):.2%}",
        "  *(отношение суммарного времени занятости маршрутов к длительности моделирования;"
        " при пакетном движении может превышать 100%)*",
    ]
...
# Строки, которые скрываются когда оба значения равны нулю
# (нет сбоев - метрики не применимы к базовым Demo-сценариям)
_ZERO_HIDE_KEYS = {
    "packet_split_delay_s",
    "cascade_delay_s",
    "recovery_time_s",
}
...
def generate_methodology_comparison_report(
    ...
    Строки с нулевыми значениями для обеих методик (неприменимые метрики)
    ...
    for key, label, _unit, fmt in _COMPARISON_METRICS:
        ...
        # Скрыть строки с нулями для неприменимых метрик (нет сбоев в сценарии)
        if key in _ZERO_HIDE_KEYS and not a_nan and not b_nan:
    ...
    lines.extend([
        ...
        "",
        "> **Примечание:** *Суммарная загрузка маршрутов горловины* рассчитывается",
        "> как отношение суммарного времени занятости всех маршрутов к длительности",
        "> моделирования. При одновременном использовании нескольких параллельных",
        "> маршрутов в режиме пакетного движения значение закономерно превышает 100%.",
        "",
        "---",
        "*Отчёт сгенерирован автоматически PIMPS Simulation Engine.*",
    ])
